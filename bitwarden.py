#!/usr/bin/env python3
"""
Optimized rofi Bitwarden password manager with caching
"""
import json
import os
import subprocess
...
class BitwardenItem:
    ...
    def display(self) -> str:
        """Return display format for rofi"""
        return f"{self.name} [{self.username}]"


class BitwardenManager:
    """Manages Bitwarden vault interactions via rofi"""

    CACHE_TIME = 86400  # 24 hours

    def __init__(self):
        self.rofi_args = ["-dmenu", "-i", "-l", "15", "-p", "Bitwarden:"]

        # Setup cache directory
        cache_home = os.getenv("XDG_CACHE_HOME", str(Path.home() / ".cache"))
        self.cache_dir = Path(cache_home) / "rofi_pass"
    ...
    def _rofi(self, items: List[str], prompt: Optional[str] = None) -> Optional[str]:
        """Run rofi with given items"""
        args = ["rofi"] + self.rofi_args
        if prompt:
    ...
    def _ensure_login(self) -> None:
        ...
        check = subprocess.run(
            ["bw", "login", "--check"], capture_output=True, check=False
        )
    ...
    def _unlock_session(self) -> None:
        ...
        if "BW_PASSWORD" in os.environ:
            ...
            result = subprocess.run(
                ["bw", "unlock", "--raw"], capture_output=True, text=True, check=False
            )
    ...
    def _refresh_cache_async(self) -> None:
        ...
        if self._cache_expired(self.items_cache):
            thread = threading.Thread(
                target=self._load_items, args=(True,), daemon=True
            )
            thread.start()
    ...
    def _auto_type(self, text: str) -> None:
        ...
        subprocess.run(
            ["xdotool", "type", "--delay", "50", "--clearmodifiers", text], check=False
        )

    def add_login(self) -> None:
        """Add new login item"""
        name = self._rofi([], "New Entry Name:")
        if not name:
            return

        username = self._rofi([], "Username:")
        password = self._rofi([], "Password:")
    ...
    def view_details(self) -> None:
        ...
        formatted = subprocess.run(
            ["jq", "."], input=details, text=True, capture_output=True, check=False
        ).stdout

        subprocess.run(["less"], input=formatted, text=True, check=False)

    def handle_action(self, selected_item: Optional[BitwardenItem] = None) -> None:
        ...
        action = self._rofi(actions, f"{item.name} [{item.username}]")
        if not action:
            return
    ...
    def run(self) -> None:
        ...
        menu_items.extend(
            ["", "[+] Add New Login", "[-] Delete Login", "[🔍] View Details"]
        )

        selection = self._rofi(menu_items)
        if not selection:
            return
        ...
        if selection == "[+] Add New Login":
            ...
            # Find the selected item and handle action
            item = next((i for i in items if i.display() == selection), None)
            if item:
